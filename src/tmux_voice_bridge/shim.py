    ...
    if m:
        ...
        if token in hosts:
            ...
            if warning:
                msg = f"{warning} {msg}"
            return msg
        # First word looked like a switch verb but the token isn't a known host
        # ("use my GTD inbox", "go to line 42"). Fall through to text injection
        # rather than yelling about an "unknown target".
